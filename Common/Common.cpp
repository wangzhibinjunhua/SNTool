...
IniData_struct g_CustomConfigIniData[] =
{
	...
	{"DB_SQLSrc", (char*)g_sMetaComm.strDBSQLSrc, INI_STRING },
...
//add by wzb for sql function 20190322
...
int SelectSqlByType(int type)
{
	...
	case SELECT_TPINPUT:
		if (!pRecordset->GetadoEOF())
		{
			...
			//AfxMessageBox("sn tpinput exist");
		}
		else
		{
	...
	case SELECT_TPPLAN:
		...
		{
			iRet = SELECT_SMO_OK;
			_variant_t varCCFlag;
			_variant_t varDetailModel;
			
			try
			{
				varCCFlag = pRecordset->GetCollect("CCFlag");
				varDetailModel = pRecordset->GetCollect("DetailModel");
				CString strCCFlag = (char *)_bstr_t(varCCFlag);
				CString strDetailModel = (char *)_bstr_t(varDetailModel);
				strCCFlag.TrimRight();
			...
			{
				CString strError;
				strError.Format(_T("读取数据失败\n %s:%s"), e.ErrorMessage(), (LPCTSTR)e.Description());
				AfxMessageBox(strError);
				pRecordset.Release();
				return SELECT_SMO_FAIL;
			}
			
		}
		else
		{
			iRet = SELECT_SMO_FAIL;
		}
		break;
	default:
		break;
